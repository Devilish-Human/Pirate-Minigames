	...
	if profile then
		...
		if profile.Coins >= itemToPurchase:GetAttribute("Cost") then
			if self:AlreadyOwnsItem(player, itemToPurchase) then
				self.Client.PrompEvent:Fire(
					player,
					"Error",
					"Already own item!",
					("You already own this item."):format(itemName)
				)
				return
			end
			...
			self.Client.PrompEvent:Fire(
				...
				"Error",
				"Not enough coins.",
				("You do not have enough coins to buy %s. Try again later when you have enough coins."):format(itemName)
			)
			print(
				("Purchased failed not enough coins. Need %s more coins"):format(
					itemToPurchase:GetAttribute("Cost") - profile.Coins
				)
			)
		end
	end
...
function ShopService:AlreadyOwnsItem(player, item)
	if
		InventoryService:GetInventory(player):FindFirstChild(item:GetAttribute("Category")):FindFirstChild(item.Name)
	then
		return true
	end
	return false

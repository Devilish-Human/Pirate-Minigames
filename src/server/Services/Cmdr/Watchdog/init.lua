--!strict
--[[ info...	Credits to Hiatus/ApprenticeOfMadara

	Add security to your code with ease.
	Handles player banning/kicking/noting (server only)
	
	This module is supposed to be used to note down problems with server code e.g.
		when an event is fired and things don't add up.
	You can note the problem down easily and kick/ban if u have to,
	so it's nothing like hd admin or any other admin thing.

	Important Notes:
	
	- This module is the same as the standard Watchdog version but without the embedded ChatCmds.
	
	- You must call Watchdog.Verify(player) on every player that joins.
			This ensures that banned players are kicked from the game,
			and players whose ban duration is up, are unbanned.

	- Moderators can be added by adding them to the DefaultModerator table (UserId = Name format),
			or by calling Watchdog.AddMod(new_mod)
			
	- To remove moderators, either remove them from the DefaultModerator list (if they were added there),
			or by calling Watchdog.RemoveMod(old_mod).
				
	- Watchdog.UpdateMods() must be called at least once when the server starts. This is done for
			you at the bottom of the module.
	
	- Ban duration unit is seconds. For indefinite bans, set the duration to -1.
	
	- Make sure commands that write to datastores are not used on a particular User in a real game setting;
			otherwise, a lot of datastore requests will be made, doing the exact same thing.
	...
	API:
		...
		# Watchdog.AddMod(new_moderator : User)
		
		# Watchdog.RemoveMod(old_moderator : User)
		
		# Watchdog.GetMods() : {[UserId] : string}
		
		# Watchdog.UpdateMods()
				=> Resyncs the 'Moderators' table with the 'DefaultModerators' and the moderators added
						using Watchdog.AddMod()
		
		# Watchdog.GetLogs(user : User, category : LogCategory?, log_number : number?)
		...
		# Watchdog.Note(user : User, moderator : User, note : string)
				=> 'moderator' is used for logging purposes.
		
		# Watchdog.RemoveNote(user : User, note_id : number)
		
		# Watchdog.Kick(user : User, moderator : User, reason : string?, format : Format?)
				=> 'moderator' is used for logging purposes.
		...
		# Watchdog.Ban(user : User, moderator : User, duration : number, reason : string?)
				=> 'moderator' is used for logging purposes.
				=> 'duration' is in seconds (set to -1 for indefinite)
				=> 'reason' will default to "None" when nil (changeable)
				

		# Watchdog.Unban(id : number, moderator : User, reason : string?)
				=> 'moderator' is used for logging purposes.
				=> 'reason' will default to "No reason given" when nil
				
		# Watchdog.LocalNote(user : User, moderator : User, note : string)
				=> Creates a note on the server. It will not be stored to any datastore and will
					disappear when the server closes.
				=> To see local notes, use Watchdog.GetLocalNotes()
				=> 'moderator' is used for logging purposes.
